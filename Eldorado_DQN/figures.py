...
def plots(ticks, x, label, idx, path, split):
    colors = Neon.color12()
    loglib.dark()
    for idx, item in enumerate(x.items()):
        ...
        loglib.plot(val, inds=tcks, label=str(annID), c=c.norm)
    loglib.godsword()
    loglib.save(path + label + '.png')
    plt.close()
...
def popPlots(popLogs, path, split):
    ...
    for key, val in popLogs.items():
        ...
        plots(ticks, val, str(key), idx, path, split)
        idx += 1
...
def individual(blobs, logDir, name, accum, split):
    ...
    for annID, blobList in blobs.items():
        ...
        logs = {**logs, **InkWell.shareFood(blobList)}
        logs = {**logs, **InkWell.shareWater(blobList)}
        logs = {**logs, **InkWell.contact(blobList)}
        logs = {**logs, **InkWell.attack(blobList)}
        logs = {**logs, **InkWell.punishAttack(blobList)}
        logs = {**logs, **InkWell.punishWater(blobList)}
        logs = {**logs, **InkWell.punishFood(blobList)}
        popLogs[annID] = logs
