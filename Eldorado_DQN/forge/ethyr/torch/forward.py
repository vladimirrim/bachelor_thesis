...
class Forward:
    ...
    def forward(self, sample_of_samples, weights, ann, targetAnn, lawmaker, device='cpu'):
        ...
        flat = torch.cat([samples[0][0]['flat'] for samples in sample_of_samples], dim=0)
        ents = torch.cat([samples[0][0]['ents'] for samples in sample_of_samples], dim=0)
        v = torch.cat([samples[0][4] for samples in sample_of_samples], dim=0)
        atnArgs, val = ann(flat.to(device), ents.to(device), v=v.to(device), device=device)

        flat = torch.cat([samples[-1][0]['flat'] for samples in sample_of_samples], dim=0)
        ents = torch.cat([samples[-1][0]['ents'] for samples in sample_of_samples], dim=0)
        v = torch.cat([samples[-1][4] for samples in sample_of_samples], dim=0)
        atnArgs_n, val_n = ann(flat.to(device), ents.to(device), v=v.to(device), device=device)
        atnArgs_nt, val_nt = targetAnn(flat.to(device), ents.to(device), v=v.to(device), device=device)
        punishments = lawmaker(flat.to(device), ents.to(device), device=device)[1]
...
class ForwardLm(Forward):
    ...
    def forward(self, sample_of_samples, weights, anns, lawmaker, device='cpu'):
        ...
        for j, samples in enumerate(sample_of_samples):
            ...
            for ent in ents:
                lst = samples[0][ent]
                vals.append(anns[lst[1]].valNet(lst[0]['flat'].to(device), lst[0]['ents'].to(device),
                                                device=device).view(-1).detach().to('cpu'))

            if self.config.LM_FUNCTION == 'sum':
                val = sum(vals)
            else:
                if self.config.LM_FUNCTION == 'min':
                    idx = np.argmin([val.mean().numpy() for val in vals]).item()
                elif self.config.LM_FUNCTION == 'max':
                    idx = np.argmax([val.mean().numpy() for val in vals]).item()
                val = vals[idx]
            ...
            for ent in ents:
                ...
                if ent in samples[-1].keys():
                    lst = samples[-1][ent]
                    if not lst[4]:
                        val_n = anns[lst[1]].valNet(lst[0]['flat'].to(device), lst[0]['ents'].to(device),
                                                    device=device).view(-1).detach().to('cpu')
            ...
            if self.config.LM_FUNCTION == 'sum':
                ...
                if self.config.LM_FUNCTION == 'min':
                    idx = np.argmin([t.mean().numpy() for t in target]).item()
                elif self.config.LM_FUNCTION == 'max':
                    idx = np.argmax([t.mean().numpy() for t in target]).item()
                target = target[idx]
            ...
            for ent in ents:
                ...
                atnArgs = lawmaker(s['flat'].to(device), s['ents'].to(device), device=device)[0]
                for key in a.keys():
