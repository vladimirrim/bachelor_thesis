...
class NativeServer:
    def __init__(self, config, args):
        self.envs = [core.Realm.remote(config, args, i) for i in range(args.nRealm)]
...
class Native(Blacksmith):
    ...
    def run(self):
        ...
        pantheonProcessId = self.run_pantheon.remote(self, sharedReplay, sharedStateDict)
        if self.args.lm:
    ...
    @ray.remote
    def run_pantheon(self, sharedReplay, sharedStateDict):
        ...
        while True:
            ...
            states, priorities = pantheon.step(sample, weights)
