...
class Saver:
    ...
    def checkpointLawmaker(self, params, opt):
        if self.epoch % 10 == 0:
            self.save(params, opt, 'lawmaker')
        if self.epoch % 500 == 0:
            self.save(params, opt, 'lawmaker' + str(self.epoch))

    def load(self, opt, anns, lmOpt, lawmaker, best=False, lm=False):
        ...
        if lm:
            ...
            try:
                data = torch.load(self.root + fname + self.extn)
                [ann.load_state_dict(st_dict) for ann, st_dict in zip(lawmaker, data['params'])]
