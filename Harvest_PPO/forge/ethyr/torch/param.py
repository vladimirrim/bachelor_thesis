...
def getGrads(ann):
    ...
    for param, e in ann.named_parameters():
        try:
            ...
            print(
                'Gradient dimension mismatch. This usually means you have either (1) loaded a model with a different architecture or (2) have a layer for which gradients are not available (e.g. not differentiable or more commonly not being used)')
    return ret
